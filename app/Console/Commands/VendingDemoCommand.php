...
final class VendingDemoCommand extends Command
{
    ...
    public function handle(
        ...
        $format = static fn (array $coins): string => $coins === [] ? '(no change)' : implode(', ', $coins);
        ...
        try {
            ...
            $this->line("Example 1  -> {$r1->product}, change: {$format($r1->change)}");

            $insert->execute(InsertCoinRequest::fromValue(0.10));
            $insert->execute(InsertCoinRequest::fromValue(0.10));
            $r2 = $returnCoins->execute();
            $this->line("Example 2  -> returned: {$format($r2->coins)}");

            $insert->execute(InsertCoinRequest::fromValue(1));
            $r3 = $select->execute(SelectProductRequest::fromCode('WATER'));
            $this->line("Example 3  -> {$r3->product}, change: {$format($r3->change)}");
